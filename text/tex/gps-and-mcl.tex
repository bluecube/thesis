...
\begin{algorithm}[H]
    ...
    \function{observationProbabilityGPS}{
        ...
        \drmsSquared \assign \( ( \WilsonParamA\hdop )^2 + \WilsonParamB^2 \)\;
        \BlankLine
...
\subsection{Sensor Model}
As with the Position domain approach, the sensor model was derived from
...
\begin{algorithm}[H]
    ...
    \function{observationProbabilityGPS}{
        ...
        \If{\abs{\rangeError} \(<\) \SI{150}{\meter}}{
            \result \assign \result \(*\) \normpdf{\rangeError, \(\mu\) = \pseudorangeErrorMu, \(\sigma\) = \pseudorangeErrorSigma}\;
        }
        \velocityError \assign \velocity \(-\) \geomVelocity\;
        \If{\abs{\velocityError} \(<\) \SI{4}{\meter\per\second}}{
            \result \assign \result \(*\) \normpdf{\velocityError, \(\mu\) = \velocityErrorMu, \(\sigma\) = \velocityErrorSigma}\;
        }
        \Return \result\;
    }
\end{algorithm}
