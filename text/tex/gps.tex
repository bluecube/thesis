...
segment and user receivers, but breakdown of error values is otherwise hard to measure
without survey hardware.

\subsubsection{Receiver Noise and Fix Accuracy}
\todo{Write something}
...
When used for navigation, satellites may be dropped from the solution based on the elevation angle
to avoid this kind of errors.
More involved method of dealing with multipath errors is described in [TODO\todo{Find the article!}],
where the error model of a satellite measurement is modified based if it is suspected to be corrupted
by multipath error.
