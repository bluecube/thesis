...
Distance between the real receiver position and the reported position is modelled using Rayleigh distribution
...
Theoretically, mean error should be equal to \(\HDOP\) times \(\UERE\) (see \autoref{sec:gps-errors}),
but in practice it is not linear.
In figure \ref{fig:wgs84_hdop_error} green line shows fitting of the theoretical linear model and
