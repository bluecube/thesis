...
%	\begin{equation}
%		\Prob(X_i = x | X_{1, \dotsc, i - 1}, a_{1, \dotsc, i - 1},
%		o_{1, \dotsc, i - 1}) = 
%		\Prob(X_i = x | X_{i - 1}, a_{i - 1})
%	\end{equation}
and that each observation only depends on the current state.
%	\begin{equation}
%		\Prob(o_i | X_1, \dotsc, X_i, a_1, \dotsc, a_{i - 1},
%		o_1, \dotsc, o_{i - 1}) = 
%		\Prob(o_i | X_i)
%	\end{equation}
...
\begin{equation}
	...
	\Bel(X_i = x) = \Prob(X_i = x | o_{1, \dotsc, i}, a_{1,\dotsc,i - 1})
\end{equation}
...
\begin{equation}
	...
	\Bel(X_i = x) =
	\frac{
		\Prob(o_i | X_i = x, o_{1, \dotsc, i}, a_{1,\dotsc,i - 1})
		\Prob(X_i = x | o_{1, \dotsc, i}, a_{1, \dotsc, i - 1})
	}{
		\Prob(o_i | o_{1, \dotsc, i-1}, a_{1, \dotsc, i-1})
	}
\end{equation}

Note that denominator \(\Prob(o_i | o_{1, \dotsc, i-1}, a_{1, \dotsc, i-1})\) only
...
\begin{multline}
	...
	\Prob(X_i = x | o_{1, \dotsc, i}, a_{1, \dotsc, i - 1}) = \\
	\int
	\Prob(X_i = x | X_{i - 1}=x', o_{1, \dotsc, i}, a_{1, \dotsc, i - 1})
	\Prob(X_{i-1} = x' | o_{1, \dotsc, i-1}, a_{1, \dotsc, i-1})
	\,\mathrm{d}x'
\end{multline}
...
We will call the conditional density \(\Prob(o_i | X_i = x)\) \emph{sensor model} and
the density \(\Prob(X_i = x | X_{i - 1}=x', a_{i - 1})\) \emph{motion model}.
...
\begin{gather}
	...
	\Prob(X_i = x | X_{i-1} = x', a_{i - 1}) \sim \mathrm{N}(A_{i - 1}x' + B_{i - 1}a_{i - 1}, Q_{i - 1})
	\\
	\label{eq:kalman-measurement-model}
	\Prob(o_{i} | X_i = x) \sim \mathrm{N}(H_{i}x, R_i)
\end{gather}
...
\(\Prob(X_i | o_{1, \dotsc, i}, a_{1, \dotsc, i - 1})\)
...
\item[Prediction] \hfill \\
	...
	from the action model \(\Prob(X_i = x | X_{i-1} = s^k_{i-1}, a_{i - 1})\)
...
\item[Correction] \hfill \\
	...
	\begin{equation}
		w^k_i = 
		\frac{
			\Prob(o_i | X_i = s'^k_i) \hat{\Prob}(X_i = s'^k_i | X_{i - 1}, a_{i - 1})
		}{
			\hat{\Prob}(X_i =  s'^k_i | X_{i - 1}, a_{i - 1})
		} = \Prob(o_i | X_i = s'^k_i)
	\end{equation}
