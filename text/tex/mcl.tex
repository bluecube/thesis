...
\(P(X_i = x \mid X_{i-1}, o_i, a_{i - 1})\) to represent the belief.
...
\begin{lstlisting}[float=b, label=lst:mcl-algo, caption={Pseudocode of prediction and correction
steps of Monte Carlo localization}]
def prediction(S, action):
	for s in S:
		old_weight = s.weight
		s = draw_from_action_model(s, action)
		s.weight = old_weight

def correction(S, observed):
	for s in S:
		s.weight *= observation_probability(s, observed)
...
%		s = new_sample_from_map()
%		s.weight = 1 / sample_count
%		S.append(1)
%	return S
...
%		cdf.append(cdf[i] + S[i].weight)
%
%
%	original_S = deepcopy(S)
%	it = iter(original_S)
%	offset = random(0, 1 / len(S))
%
%	for i in range(len(S)):
%\end{lstlisting}

\autoref{lst:mcl-algo} contains a simple pseudocode implementation of the algorithm.
Function \lstinline=draw_from_action_model()= represents drawing a new sample from density
the action model and the function \lstinline=observation_probability()=
contains the observation model and returns \(\Prob(o_i \mid X_i = s'^k_i)\).
...
Pseudocode in \autoref{lst:mcl-algo} extends the basic MCL algorithm by keeping sample weights during the
whole operation.
...
Criterion for starting resampling may be chosen as a fixed number of prediction / correction steps,
or when the effective number of particles drops under a predefined threshold.
